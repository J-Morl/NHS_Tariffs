...
creating a stored function to calculate the tariff for the randomised hes data
	issue with this data is randomness also applies to columns it shouldnt such as admiage not matching up
	therefore going to use the min admiage, min classpat within each spell but group them by their spell
...
IF EXISTS (SELECT * FROM sysobjects WHERE ID=OBJECT_ID(N'tariff.usp_1819TariffCalc') and OBJECTPROPERTY(id, N'IsProcedure')=1)
    BEGIN
        DROP Procedure tariff.usp_1819TariffCalc
    END
GO

CREATE PROCEDURE tariff.usp_1819TariffCalc
@tablename nvarchar(max)
as
BEGIN
SET NOCOUNT ON;
IF COL_LENGTH(''+@tablename+'', 'Tariff 18_19 (£) for spell') IS NOT NULL
BEGIN
declare @command1 nvarchar(max)
set @command1 = '
   ALTER TABLE '+@tablename+'
   DROP COLUMN [Tariff 18_19 (£) for spell]
'
...
set @command2 = '
alter table '+@tablename+'
add [Tariff 18_19 (£) for spell] int
'
...
select
	...
	end as [Tariff 18_19 (£) for spell]
into #temp
from rgd
	left join tariff.[Clean_18-19] c
		on rgd.HRG_code = c.[HRG code]

update '+@tablename+'
set '+@tablename+'.[Tariff 18_19 (£) for spell] = #temp.[Tariff 18_19 (£) for spell]
from #temp
